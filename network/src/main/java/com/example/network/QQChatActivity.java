 ...
 */
public class QQChatActivity extends ActivityGroup implements OnClickListener {
	...
	private void changeContainerView(View v) {
		...
		v.setSelected(true);
		if (v == ll_first) {
			toActivity("first", QQConcernActivity.class);
		} else if (v == ll_second) {
			toActivity("second", QQContactActivity.class);
		} else if (v == ll_third) {
			toActivity("third", QQFindActivity.class);
		}
	}
